#!/usr/bin/env bash
#
# 008-vpc-subnets 受入テスト雛形
#
# 前提:
#   1. terraform/ 配下で `terraform apply` 済みであること
#   2. outputs.tf に、101までの5つの値に加えて以下を出力しておくこと
...
if [[ -z "$WEB_IP" || -z "$INSTANCE_ID" || -z "$SG_ID" || -z "$BASTION_INSTANCE_ID" || -z "$BASTION_SG_ID" || -z "$PUBLIC_SUBNET_ID" || -z "$PRIVATE_SUBNET_ID" ]]; then
  echo "terraform output が取得できません。outputs.tf の定義を確認してください。"
  exit 1
fi
echo "Web: $INSTANCE_ID / Bastion: $BASTION_INSTANCE_ID / PublicSubnet: $PUBLIC_SUBNET_ID / PrivateSubnet: $PRIVATE_SUBNET_ID"
echo
...
echo "== テスト4: プライベートサブネットがIGWへの経路を持たないか =="
# サブネットに明示的なアソシエーションがない場合はVPCのメインルートテーブルが使われるため、
# association.subnet-id だけでなく、メインルートテーブルへのフォールバックも考慮する。
...
PRIVATE_IGW_ROUTE=$(aws ec2 describe-route-tables \
  --route-table-ids "$PRIVATE_RT_ID" \
  --query "RouteTables[0].Routes[?starts_with(GatewayId, 'igw-')]" \
  --output text)
if [[ -z "$PRIVATE_IGW_ROUTE" ]]; then
  pass "プライベートサブネット（RT: $PRIVATE_RT_ID）にIGWへの経路はない"
else
  fail "プライベートサブネットにIGWへの経路が見つかりました: $PRIVATE_IGW_ROUTE"
fi
echo
